...
const SPEC = {
  ...
  name_ar: 'الجهاز الهضمي والأعضاء الداخلية',
};

/** Public Supabase URL — run scripts/upload-speciality-images.mjs after deploy. */
const GASTROENTEROLOGY_SUPABASE_IMAGE_URL =
  'https://hjluqxfmvpvtjvwzqxgi.supabase.co/storage/v1/object/public/files/static/specialities/gastroenterology.png';

export class GastroenterologySpeciality1778820000000
  implements MigrationInterface
{
  name = 'GastroenterologySpeciality1778820000000';

  public async up(queryRunner: QueryRunner): Promise<void> {
    const bundled = SPECIALITY_IMAGES[SPEC.name_en];
    const imageUrl = GASTROENTEROLOGY_SUPABASE_IMAGE_URL || bundled;
    if (!imageUrl) {
