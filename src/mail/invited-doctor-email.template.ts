...
export function buildInvitedDoctorEmailHtml(input: {
  language: MarketingEmailLanguage;
  ...
  const rawSections =
    input.sections?.length && input.sections
      ? input.sections
      : getDefaultInvitedDoctorSections(input.language);
  ...
  return {
    subject: invitedDoctorSubject(input.language, vars.name),
